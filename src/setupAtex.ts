  ...
  try {
    ...
    if (resultatCredDef.credentialDefinitionState.state === 'failed') {
      ...
      if (motiu.includes('already exists') || motiu.includes('SeqNo')) {
        console.log('[ok] CredDef ATEX ja estava registrada.')
      } else {
        throw new Error(`CredDef fallida: ${motiu}`)
      }
    } else {
      console.log('\n[ok] CredDef ATEX registrada!')
      console.log('================================================')
      console.log(`ID CREDDEF ATEX: ${resultatCredDef.credentialDefinitionState.credentialDefinitionId}`)
      console.log('================================================')
      console.log('\n⚠️  Copia aquest ID i actualitza CRED_DEF_ATEX a src/configuracio.ts')
    }

    await agent.shutdown()
    console.log('\n--> Setup ATEX completat. Apagat net.')
    process.exit(0)
